...
/**
* Crea una clase MeyorDelVector que cree un vector con enteros aleatorios y muestre el
* valor mínimo. Para ello, implementa los métodos pedirDimension(),
* inicializarVectorAleatoriamente(int[], i, max) y hallarMinimo(int[]).
*/
...
public class MenorDelVector{
	...
	*/
	...
	/**
	* Halla el valor mínimo del vector
	* @param: vector: El vector con el que se va a trabajar
	*/
	static void hallarMinimo(int[] vector){
		Arrays.sort(vector);
		System.out.println("El valor mínimo del vector es " + vector[0]);
	}
}